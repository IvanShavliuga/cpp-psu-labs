...
void DelString(void)
{
   register char y;  //═юьхЁ ёЄЁюъш
   register int p;   //═юьхЁ ёЄЁрэшЎ√
   unsigned long int part;
   char buf[80],str[80];
   HintView(PLEASEWAIT);
   //─╦▀ ╥┼╩╙┘┼╔ ╤╥╨└═╚╓█
   //╩юяшЁютрэшх ёыхфє■∙хщ ёЄЁюъш т Єхъє∙є■
   for(y=Row; y<22;y++)
     strncpy(Text[Page][y],Text[Page][y+1],80);
   //─╦▀ ╤╦┼─╙▐┘┼╔ ╤╥╨└═╚╓█ (┼╤╦╚ ╬═└ ┼╤╥▄)
   //╩юяшЁютрэшх яхЁтющ ёЄЁюъш шч ёыхфє■∙хщ ёЄЁюрэшЎ√ т
   //яЁхф√фє∙є■ ёЄЁрэшЎє
   if(CountPages>1)
     strncpy(Text[Page][21],Text[Page+1][0],80);
   //╥юўэю Єръ цх фы  юёЄры№э√ї ёЄЁрэшЎ
   if(!CountPart)
   {
     for(p=Page+1; p<=CountPages; p++)
     {
